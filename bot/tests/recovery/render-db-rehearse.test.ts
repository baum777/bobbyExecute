...
describe("Render-side rehearsal refresh", () => {
  it("runs the disposable rehearsal path with automated Render metadata", async () => {
    ...
    const buildEvidenceRepository = vi.fn(() => ({
      ...
      listDatabaseRehearsalEvidence: async () => [],
      loadDatabaseRehearsalFreshnessAlert: async () => null,
      saveDatabaseRehearsalFreshnessAlert: async () => undefined,
      recordDatabaseRehearsalFreshnessAlertEvent: async () => undefined,
      listDatabaseRehearsalFreshnessAlertEvents: async () => [],
